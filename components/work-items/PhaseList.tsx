...
function draftFromPhase(phase: WorkItemPhaseRow): PhaseDraft {
  return {
    name: phase.name,
    ownerMemberId: phase.owner_member_id ?? UNASSIGNED,
...
export function PhaseList({
  ...
  function toOwnerId(value: string): string | null {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const res = await createPhaseAction({
        ...
        ownerMemberId: toOwnerId(addDraft.ownerMemberId),
      });
      if (!res.ok) {
  ...
  function handleSave(phaseId: string) {
    ...
    startTransition(async () => {
      const res = await updatePhaseAction({
        ...
        ownerMemberId: toOwnerId(draft.ownerMemberId),
      });
      if (!res.ok) {
        setErrorMessage(res.message);
        return;
    ...
    <section className="space-y-3">
          ...
          {phases.map((phase, index) => {
            ...
            return (
              <li key={phase.id} className="p-3">
                {isEditing ? (
                  <div className="space-y-3">
                    {renderFields(draft, setDraft, `phase-${phase.id}`)}
      ...
      {isAdding ? (
        <div className="space-y-3 rounded-md border bg-muted/20 p-3">
          {renderFields(addDraft, setAddDraft, "phase-new")}
          <div className="flex justify-end gap-2">
            <Button
              type="button"
              variant="ghost"
